...
import { Film, Manufacturer, MonthlyReportPlanProps } from "@/types";
...
const Tab1 = () => {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["quarterly-report-plan", filterValues],
    queryFn: () => {
      ...
      return getQuarterlyReport(payload).then(
        (res) => res as MonthlyReportPlanProps,
      );
    },
  ...
  const getAllRooms = (data: Manufacturer[]) => {
    ...
    data.forEach((m) =>
      m.films.forEach((f) => f.rooms.forEach((r) => set.add(r.roomName))),
    );
  ...
  const rooms = useMemo(() => getAllRooms(data?.data || []), [data?.data]);

  const dataSource = useMemo(
    () => buildTreeTableData(data?.data || [], rooms),

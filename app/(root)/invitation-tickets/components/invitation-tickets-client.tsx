...
const InvitationTicketsClient = () => {
  ...
  const { data: backgrounds, isFetching: isFetchingBackgrounds } = useQuery({
    queryKey: ["backgrounds"],
    queryFn: getBackgrounds,
    placeholderData: keepPreviousData,
  });

  const { data: invitationTickets, isFetching } = useQuery({
    queryKey: ["invitation-tickets", { current, pageSize, filterValues }],
    queryFn: () => {
  ...
  const columns: TableProps<OrderDetailProps>["columns"] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const chairsF1 = record.order?.items?.map(
          (item) => item.listChairValueF1,
        );
        const chairsF2 = record.order?.items?.map(
          (item) => item.listChairValueF2,
        );
        const chairsF3 = record.order?.items?.map(
          (item) => item.listChairValueF3,
        );
        const allChairs = [...chairsF1, ...chairsF2, ...chairsF3].filter(
          Boolean,
        );

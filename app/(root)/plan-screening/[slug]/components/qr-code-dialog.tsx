"use client";

import {
  Dialog,
  DialogContent,
  DialogFooter,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { formatMoney } from "@/lib/utils";
import { format } from "date-fns";
import { formatInTimeZone } from "date-fns-tz";
import { Hourglass } from "lucide-react";
import QRCode from "react-qr-code";

interface QrCodeDialogProps {
  open: boolean;
  onOpenChange: (newOpen: boolean) => void;
  filmName: string;
  roomName: string;
  projectDate: string;
  projectTime: string;
  qrCode: string;
  accountName: string;
  accountNumber: string;
  totalAmount: number;
  selectedSeats?: string;
}

const QrCodeDialog = ({
  open,
  onOpenChange,
  filmName,
  roomName,
  projectDate,
  projectTime,
  qrCode,
  accountName,
  accountNumber,
  totalAmount,
  selectedSeats,
}: QrCodeDialogProps) => {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[808px]">
        <DialogHeader className="border-b">
          <DialogTitle className="text-xl">Thanh toán QR Code</DialogTitle>
        </DialogHeader>

        <div className="px-6 py-9">
          <div className="flex gap-6">
            <div className="w-3/5">
              <div className="bg-goku rounded-lg p-4">
                <p className="font-bold mb-4">Thông tin vé</p>
                <div className="space-y-2">
                  <div className="flex justify-between gap-4 text-sm">
                    <p className="w-[200px] text-trunks">Tên phim đã chọn</p>
                    <p className="flex-1 font-bold">{filmName}</p>
                  </div>
                  <div className="flex justify-between gap-4 text-sm">
                    <p className="w-[200px] text-trunks">Vị trí ghế</p>
                    <p className="flex-1 font-bold text-whis">
                      {selectedSeats}
                    </p>
                  </div>
                  <div className="flex justify-between gap-4 text-sm">
                    <p className="w-[200px] text-trunks">Phòng chiếu</p>
                    <p className="flex-1 font-bold">{roomName}</p>
                  </div>
                  <div className="flex justify-between gap-4 text-sm">
                    <p className="w-[200px] text-trunks">Ngày giờ</p>
                    <p className="flex-1 font-bold">
                      {format(new Date(projectDate), "dd/MM/yyyy")}-{" "}
                      {formatInTimeZone(projectTime, "UTC", "HH:mm")}
                    </p>
                  </div>
                </div>
              </div>

              <div className="bg-krillin/10 rounded-lg p-4 mt-10 flex items-center justify-between">
                <div>
                  <p className="font-bold text-sm">
                    Bạn còn <span className="text-chichi">[04:59]</span> để
                    thanh toán
                  </p>
                  <p className="mt-1 text-trunks text-sm">
                    Đang chờ thanh toán ...
                  </p>
                </div>
                <Hourglass size={36} className="text-krillin" />
              </div>
            </div>
            <div className="w-2/5">
              <div className="flex flex-col items-center">
                <div className="p-5 relative bg-white">
                  <QRCode value={qrCode} size={200} />
                  <div className="absolute inset-0 pointer-events-none flex justify-between items-between">
                    <div className="absolute top-0 left-0 w-6 h-6 border-t-4 border-l-4 border-primary rounded-tl-2xl"></div>
                    <div className="absolute top-0 right-0 w-6 h-6 border-t-4 border-r-4 border-primary rounded-tr-2xl"></div>
                    <div className="absolute bottom-0 left-0 w-6 h-6 border-b-4 border-l-4 border-primary rounded-bl-2xl"></div>
                    <div className="absolute bottom-0 right-0 w-6 h-6 border-b-4 border-r-4 border-primary rounded-br-2xl"></div>
                  </div>
                </div>

                <p className="text-center font-bold text-base mt-4">
                  {accountNumber}
                </p>
                <p className="text-center font-bold text-base">
                  {accountName}
                </p>
              </div>
            </div>
          </div>
        </div>

        <DialogFooter>
          <div className="flex w-full">
            <div className="w-3/5 text-sm">
              <p className="text-trunks">
                Tổng tiền vé:{" "}
                <span className="font-bold text-black">
                  {formatMoney(totalAmount)}
                </span>
              </p>
              <p className="text-trunks mt-1">
                Phụ phí:{" "}
                <span className="font-bold text-black">{formatMoney(0)}</span>
              </p>
            </div>
            <div className="w-2/5 border-l pl-4">
              <p className="text-sm text-trunks">Tổng thanh toán</p>
              <p className="text-chichi font-bold text-xl mt-1">
                {formatMoney(totalAmount)}
              </p>
            </div>
          </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
};

export default QrCodeDialog;

...
const CancellationTicketsClient = () => {
  ...
  const { data: cancellationTickets, isFetching } = useQuery({
    queryKey: ["cancellation-tickets", { current, pageSize, filterValues }],
    queryFn: () => {
  ...
  const columns: TableProps<CancellationTicketProps>["columns"] = [
    ...
    },
    {
      ...
      render: (value: string) =>
        dayjs(value, "YYYY-MM-DD").utc().format("DD/MM/YYYY"),
    },
    {
      title: "Giờ chiếu",
      key: "projectTime",

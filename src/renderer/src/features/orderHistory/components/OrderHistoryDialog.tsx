...
const OrderHistoryDialog = ({ open, onOpenChange, selectedItem }: OrderDialogProps) => {
  return (
    <Modal title="Thông tin vé bán" open={open} onCancel={() => onOpenChange(false)} width={1000}>
      <div className="max-h-[80vh] overflow-y-auto py-5 px-6">
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="py-3 px-4">
            ...
            <p className="uppercase">
              {[selectedItem?.order.customerFirstName, selectedItem?.order.customerLastName]
                .filter(Boolean)
                .join(" ")}
            </p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-8 bg-beerus mt-4 rounded-md">
          ...
          <div className="py-3 px-4">
            ...
            <p>{selectedItem?.order.items.map((item) => item.listChairValueF1).join(", ")}</p>
          </div>
          <div className="py-3 px-4">
          ...
          <div className="py-3 px-4">
            <Button variant="outlined">Chuyển sang thành công</Button>
          </div>
        </div>
      </div>
    </Modal>

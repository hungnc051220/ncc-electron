...
type ExportRevenueSharingListParams = {
  data: ReportRevenueSharingProps[];
  summary: {
    totalRevenueNCC: number;
...
export const exportRevenueSharingListExcel = async ({
  data,
  summary,
  fromDate,
  toDate
}: ExportRevenueSharingListParams): Promise<SaveFileResult> => {
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet("Revenue Sharing");
  ...
  const fileName = sanitizeFileName(
    hasDateRange
      ? `Quản lý phân chia doanh thu ${formattedFromDate!.format("DD-MM-YYYY")}-${formattedToDate!.format("DD-MM-YYYY")}.xlsx`
      : "Quản lý phân chia doanh thu.xlsx"
  );

  ws.mergeCells(1, 1, 1, totalColumns);
  ws.getCell(1, 1).value = "QUẢN LÝ PHÂN CHIA DOANH THU";
  ...
  data.forEach((item, index) => {
    ws.addRow([
      ...
      item.premieredDay && dayjs(item.premieredDay).isValid()
        ...
        : "",
      Number(item.totalRevenue || 0) - Number(item.sharedRevenue || 0),
      Number(item.sharedRevenue || 0),
  ...
  const summaryRow = ws.addRow([
    ...
    "",
    "",
    summary.totalRevenueNCC,
    summary.totalSharedRevenue,
    summary.allRevenue
  ]);
  ...
  for (let row = bodyStartRow; row <= bodyEndRow; row++) {
    ...
    for (let col = 5; col <= 7; col++) {
      ws.getCell(row, col).alignment = { horizontal: "right", vertical: "middle" };
  ...
  for (let col = 5; col <= 7; col++) {
    ws.getCell(summaryRowNumber, col).font = { bold: true };
    ws.getCell(summaryRowNumber, col).alignment = { horizontal: "right", vertical: "middle" };
    ws.getCell(summaryRowNumber, col).numFmt = moneyFormat;
  }

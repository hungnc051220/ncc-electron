---
name: pre-push-check
description: Kiểm tra trước khi push code trong repo Electron + React + TypeScript này. Dùng khi user sắp push, muốn kiểm tra branch hiện tại, working tree, TypeScript, lint, build hoặc cần đảm bảo code an toàn trước khi đẩy lên remote GitHub/Bitbucket.
---
...
3. Chạy hoặc đề xuất chạy command thật trong `package.json`:
   ...
   - `npm run lint`
   - `npm run build`
4. Không chạy `npm test` trong gate pre-push mặc định; chỉ chạy khi user yêu cầu rõ hoặc thay đổi có rủi ro test đáng kể.

...
3. Chạy local checks bắt buộc bằng script có thật trong `package.json`:
   ...
   - `npm run lint`
   - `npm run build`
   - Không chạy `npm test` trong flow release mặc định; test chỉ chạy khi user yêu cầu hoặc khi cần xác minh phạm vi thay đổi.
...
2. **Kết quả từng bước**: pass/fail cho `typecheck`, `lint`, `build`, GitHub Actions release, artifact/release check; ghi rõ test đã bỏ qua theo quy trình mặc định.
